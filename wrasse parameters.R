...
Amax <- c(9,9, 9) #max age across all groups
#amat = c(2,3, 1) #age at  maturation; not using this right now
#q = c(0.9, 0.8,1) #age-based maturation ogive steepness, adjusted by eye (not using right now)
...
pmat[,,1]<-c(0, rep(1, Amax[1]-1)) #this fills pmat with the knife-edge maturation probability at age 2 for all females
pmat[,,2]<-c(0, rep(1, Amax[2]-1)) #this fills pmat with  0, then 1, but will be replaced with size dependent maturation probability at age 2 in the loop below
